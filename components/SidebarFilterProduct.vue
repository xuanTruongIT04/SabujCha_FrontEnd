      ...
      <div class="shop-price-filter mt-40 shop-sidebar-border pt-35">
        ...
        <div class="price_filter mt-25">
          ...
          <vue-slider
            ...
            :interval="50"
            @change="handleSlideChangeDebounce"
          ></vue-slider>
...
export default {
  ...
  methods: {
    handleSlideChangeDebounce: debounce(function (value) {
    ...
    handleSlideChange(value) {
      ...
      console.log(priceSatisfied);
      this.$emit("priceSatisfied", priceSatisfied);
    },
    async fetchData() {
      try {
        this.$bus.$emit("isLoading3Dot", true);
        const response = await this.$axios.get("/category/get-sidebar-filter");

        const resFilterSidebar = response.data.data;

        if (resFilterSidebar) {
          const resCategories = resFilterSidebar.tree_list;
          const resBrands = resFilterSidebar.brand_list;
          const resColors = resFilterSidebar.color_list;
          const resTags = resFilterSidebar.tag_list;
          const resMMPrice = resFilterSidebar.price_min_max;

          this.$bus.$emit("isLoading3Dot", false);
          let maxPrice = 1000;
          if (resMMPrice?.max_price > maxPrice) {
            maxPrice = resMMPrice?.max_price;
          }
          this.priceValueCurrent[1] = Math.ceil(maxPrice);
          this.allCategories = resCategories;
          this.allBrands = resBrands;
          this.allColors = resColors;
          this.allTags = resTags?.slice(0, 5);
