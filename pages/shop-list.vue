...
export default {
  ...
  methods: {
    ...
    async changePage(page) {
      ...
      await this.$axios
        .get(`product/list-pagination?page=${page}`)
        .then((res) => {
    ...
    async applyFilters() {
      ...
      if (this.priceRange.length > 0) {
        ...
        this.condition.min_price = minValue;
        this.condition.max_price = maxValue;
      }

      if (this.selectedBrands.length > 0) {
        this.condition.brand = this.selectedBrands;
      }

      if (this.selectedColors.length > 0) {
        this.condition.color = this.selectedColors;
      }

      if (this.selectedTag) {
        this.condition.tag = this.selectedTag;
      }

      this.isLoading = true;
      await this.$axios
        .post(`/product/filter/`, this.condition, {
          headers: {
